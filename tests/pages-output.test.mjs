...
const approvedComicAssets = {
  "p1-lettered.svg": "601855a49c019ddb39d3a9e87fe3e6d13fe27fb0406d4ff4d809061a3827b645",
  "p2-lettered.svg": "fcb6cb4e2e1a273aeb066435cee8749d441e6e61e6e24d0a8027a65a7787c1f8",
  "p3-lettered.svg": "12c0252f6bddaa9d168475a86db1eb8c3701d0f0fb164646e27dfb73e8eed0e9",
  "p4-lettered.svg": "2a1bbce56bc8062a6b96e3e65e7a6bb390bb8bef1ed93c76f89694afacf18936",
};
...
test("produces a complete GitHub Pages artifact", async () => {
  ...
  for (const filename of Object.keys(approvedComicAssets)) {
    assert.match(html, new RegExp(`comics/executive-twin/${filename}`));
  }
  assert.doesNotMatch(
    html,
  ...
  await Promise.all([
    ...
    ...Object.keys(approvedComicAssets).map((filename) =>
      access(new URL(`comics/executive-twin/${filename}`, outputRoot)),
    ),
    access(new URL(".nojekyll", outputRoot)),
    access(new URL("app/ComicReader.tsx", projectRoot)),
  ]);

  for (const [filename, expectedHash] of Object.entries(approvedComicAssets)) {
    const contents = await readFile(
      new URL(`comics/executive-twin/${filename}`, outputRoot),
    );
